...
class Player {
  constructor(nickname, familyName, mmr, availability = [], day = [], className, classMode, twitch = '@zeusGhostz') {
    if (nickname === undefined || mmr === undefined || className === undefined || classMode === undefined) throw new Error('required some information about the player');
  ...
  addMatchToHistory(matchId) {
    this.history.push(matchId);
  }
}
...
export async function stats(playerName) {
  ...
  Matches: ${players[player].stats.matches}
...
export async function updatePlayerHistoryAndStatus(player1Nickname, player2Nickname, matchId) {
  let players = await loadData();
  
  for (let player of Object.values(players)) {
    if (player.nickname === player1Nickname || player.nickname === player2Nickname) {
      player.addMatchToHistory(matchId);
      player.checkOut();
    }
  }
  
  await saveData(players);
  console.log(`Players ${player1Nickname} and ${player2Nickname} have been updated with match ${matchId}`);
}
